...
const EditRecipe = () => {
    ...
    if (recipe_id !== null) {
      axios
        ...
        .then(res => {
          ...
          setBaseRecipe(data.base_recipe);
        })
        .catch(error => {
          console.log(error);
  ...
  const handleBaseRecipeChange = e => {
    const info = base_recipe_list.filter(
  ...
  const handleImageChange = (e, index) => {
    ...
    for (let i = 0; i < files.length; i++) {
      newSteps[index].media_list.push({
        media_file: URL.createObjectURL(files[i]),
      });
    }
  ...
  const handleSubmit = async e => {
      ...
      if (step) {
        ...
        for (let media of step.media_list) {
          if (media) {
            let blob = await fetch(media.media_file).then(r => r.blob());
            payload.append(
              `step_list[${index}]media_list[${count}]media_file`,
              blob,
              media.media_file.split("/").pop()
            );
            count += 1;
          }
        }
    ...
    axios
      .put(`http://localhost:8000/recipe/${recipe_id}/edit/`, payload, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("access_token")}`,
      ...
      >
        <Stack spacing={2}>
          <List>
              ...
              <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                ...
                >
                  ...
                  {base_recipe_list.map((recipe, index) => {
                    return (
                      <MenuItem key={index} value={recipe.name}>
                        {recipe.name}
                      </MenuItem>
                    );
                  })}
            ...
            <ListItem>
              <label className="recipe-label mx-2" htmlFor={`recipe-media`}>
                  ...
                  <Stack spacing={2} className="p-3">
                    ...
                    <div>
                      ...
                      <TextField
                        ...
                        value={diet.name}
                        required
                        onChange={e => handleDietsChange(e, index)}
                      />
                    </div>
                    <button
